...
#define FINAL_T 0.3 //3.0001
#define LOGFILE "rising_output_SIMNAME/log_file.txt"
#define INTERFACE "rising_output_SIMNAME/interface.txt"
#define OUTPUTFIELD "rising_output_SIMNAME/output_field-%2.1f.txt"
...
{ 
  ...
  {
    fp1 = fopen (nameF, "a");
    fprintf(fp1, "%d %g %g %g %d ", mg.i, mg.resb, mg.resa,
...
event output_field(t = 0.0; t += 0.1; t <= FINAL_T)
{
    ...
    static FILE * fp2;
     ...
     if (i == 0) {
       ...
       foreach()
       {
         fprintf (fp2, "%f %f %f %f %f %f",x,y,u.x[],u.y[], p[], vort[]);
       }
       fprintf(fp2, "\n\n");
       fclose(fp2);
     } else {
